...
$fecha = $_POST['fecha_establecimiento'];
$id_usuario = $_POST['fecha_establecimiento'];
...
$query_user = $con->prepare("SELECT Persona FROM r_accioneseess WHERE id_usuario = ? AND DATE(fecha) = ? LIMIT 1");
...
$query_acciones = $con->prepare("SELECT id, id_usuario, accion, fecha FROM r_accioneseess WHERE id_usuario = ? AND DATE(fecha) = ?");
