...
$id_usuario = $_POST['usuario_asignacion'];
...
$query_user = $con->prepare("SELECT Persona FROM r_accionesasigna WHERE id_usuario = ? AND DATE(fecha) = ? LIMIT 1");
...
$query_acciones = $con->prepare("SELECT id, id_usuario, accion, fecha FROM r_accionesasigna WHERE id_usuario = ? AND DATE(fecha) = ?");
