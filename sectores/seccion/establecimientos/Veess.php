...
while ($row = mysqli_fetch_assoc($result)) {
    $data[] = [
        ...
        $row['nom_red'],
        $row['nom_dist'],
        '<button class="btn btn-info btn-sm editbtn" data-ideess="' . $row['ideess'] . '">Editar</button> ' .
        '<button class="btn btn-danger btn-sm deleteBtn" data-ideess="' . $row['ideess'] . '">Eliminar</button>'
    ];
    $count++;
}
