...
while ($row = mysqli_fetch_assoc($result)) {
    $data[] = [
        ...
        $row['Usuario'],
        $row['Establecimiento'],
        '<button class="btn btn-info btn-sm editbtn" data-idusuario="' . $row['ID'] . '" data-dni="'.$row['DNI'].'">Editar</button> ' .
        '<button class="btn btn-danger btn-sm deleteBtn" data-idusuario="' . $row['ID'] . '" data-dni="'.$row['DNI'].'">Eliminar</button>'
    ];
    $count++;
}
