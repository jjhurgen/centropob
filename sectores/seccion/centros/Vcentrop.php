...
while ($row = mysqli_fetch_assoc($result)) {
    $data[] = [
        ...
        $row['Distrito'],
        $row['Provincia'],
        '<button class="btn btn-info btn-sm editbtn" data-idcentro_poblado="' . $row['ID'] . '">Editar</button> ' .
        '<button class="btn btn-danger btn-sm deleteBtn" data-idcentro_poblado="' . $row['ID'] . '">Eliminar</button>'
    ];
    $count++;
}
